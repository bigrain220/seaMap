    ...
    <div class="top-btn">
      <div>
        <el-badge :value="unusualData.length" :max="99" class="badge-item" style="margin:0 20px;">
          <el-button size="small" @click="unusualClick">异常数据</el-button>
        </el-badge>
      </div>
...
export default {
    ...
    return {
      ...
      config: {
        ...
        onBeforeEventRender: function(args){
          ...
          //锁定时禁止拖动
          if(args.data.manual || args.data.status===9){
  ...
  methods: {
    ...
    selectTimeClick(type){
      ...
      this.timeForm.selectionArr=JSON.parse(arr);
    ...
    getInterval(){
      ...
      //防止多次弹框
      let dom = document.getElementsByClassName('el-message-box__wrapper')[0];
      if(dom){
        return false;
      }
      this.$confirm('检测到当前时间发生变化,更新将重置数据,是否更新?', '提示', {
  ...
  mounted() {
    ...
    let S= new Date(now).getSeconds();
    let leftTime = ((59-M)*60+(60-S))*1000;
    timer1=setTimeout(()=>{
      this.getInterval()//先自己执行一次
      timer2 = setInterval(()=>{this.getInterval()},1000*60*60);
    },leftTime)
  },
  beforeDestroy() {
    _this=null;
