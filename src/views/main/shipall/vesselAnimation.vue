<template>
  <div class="docking-wrap" :style="{'--vessel-length': vesselLength,
    ...
    <div class="docking-viewer">
      <div v-show="vesselVisible" class="vessel-wrap" ref="vesselWrap">
        <div v-if="vesselTitle" v-text="vesselTitle" class="vessel-title" />
        <div v-if="vesselText" class="vessel-text"><span v-text="vesselText" /></div>
      </div>
    </div>
    <div v-show="terminalShow" class="terminal-wrap" ref="terminalWrap"></div>
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      ...
      //组件激活状态
      activated: false,// 之前是false 改成了true
  ...
  computed: {
    ...
    imgPath() {
      return [this.vesselSvgName, this.terminalSvgName]
    },
  },
  watch: {
    disAndAng: {
    ...
    imgPath: {
      handler(newValue) {
        //改了船舶或码头的svg后 所有的都要重新计算
        let _this = this;
        let vessel = vesselsSvg[newValue[0]];
        let terminal = vesselsSvg[newValue[1]];
        Promise.all([axios.get(vessel), axios.get(terminal)]).then(_this.initObj);
      },
      deep: true
    },
    hookCables(newValue) {
  ...
  mounted() {
    let _this = this;
    this.loadSvg().then(_this.initObj);
  ...
  methods: {
     ...
     */
    loadSvg() {
      let vessel = vesselsSvg[this.vesselSvgName];
      let terminal = vesselsSvg[this.terminalSvgName];
      return Promise.all([vessel ? axios.get(vessel) : Promise.resolve(), terminal ? axios.get(terminal) : Promise.resolve()]);
    },
    /**
     * 初始化一些对象
     * @param svgTexts
    ...
    initObj(svgTexts) {
      ...
      if (vesselData) {
        _this.initVesselSVG(vesselData, vesselWrap);
        _this.vesselSvgSnap = new Snap(vesselWrap.querySelector("svg"));
      }

      if (terminalData) {
        _this.initTerminalSVG(terminalData, terminalWrap);
        _this.terminalSvgSnap = new Snap(terminalWrap.querySelector("svg"));
      }

      _this.cableSvgSnap = new Snap(cableWrap);
    },
    ...
    setHookCableGroup(groupNum, gStatus = []) {
      ...
      group.forEach((cableHook, index) => {
        ...
        if (cableHook) {
          ...
          group[index] = {
            cable: this.cableSvgSnap.paper.line(x, y, x, y)
              .attr(attr)
              .attr({ filter: status > 1 ? "url(#f1)" : null }),
    ...
    getVesselPoint(angle = this.vesselLocation?.ang,
      ...
      //如果是右舷靠泊  则船头和船尾的点要互换一下
      if (this.vesselBerthingDir + '' === '0') {
        return tailPointsDomRect.concat(headPointsDomRect);
      } else {
        return headPointsDomRect.concat(tailPointsDomRect);
      }
    },
    /**
     * 获取船头(船尖)上连接绳子的点
     * @return {[{x,y}]}
     */
    getVesselHeadPoint(fraction, vesselSnap = this.vesselSvgSnap) {
      let domRectXYList = [];
     ...
     * @return {[{x,y}]}
     */
    getVesselTailPoint(fraction, angle, vesselSnap = this.vesselSvgSnap) {
    ...
    getTerminalHookElement(group, index) {
      let stationElement = this.terminalSvgSnap?.selectAll('g.station')?.[group];
      return {
          ...
          if (cable && hook) {
            ...
            if (x1 && y1) {
              if (this.activated && this.vesselVisible) {
                let vesselPoint = this.getVesselPoint();
...
.docking-wrap {
  ...
  background: repeating-linear-gradient(
    0deg,
    transparent,
    transparent calc(var(--scale-mark) - 1px),
    #36393e,
    #36393e calc(var(--scale-mark) + 1px)
  );
}
...
.vessel-wrap > .vessel-text {
  ...
  color: #c0e3f0;
  font-size: 2rem;
  text-shadow: #939393 1px 0 3px, #000 0 1px 3px, #000 -1px 0 3px,
    #000 0 -1px 3px;
}
