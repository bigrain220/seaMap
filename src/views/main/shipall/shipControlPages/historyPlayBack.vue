...
import historyHome from "@/views/main/shipall/shipControlPages/historyHome.vue"
...
export default {
  ...
  methods: {
    ...
    displayAllDataByTime(time) {
      ...
      let {terminal, 'hook-station': hookStation} = info;
      if (!terminal) {
        return;
      }
      //改变船舶状态
      let {leftDis, rightDis, angle, mode} = terminal;
      singlePageRef.setVessel({
        vesselAngle: angle, vesselDistance: ((leftDis - 0) + (rightDis - 0)) / 2
      })
      // 直接给角度赋值
      singlePageRef.setDockAngleNum(angle);
      //直接显示在左侧
      singlePageRef.getleftShow({leftDis, rightDis, mode})
      singlePageRef.getTimeObj(this.timeObj);
      //判断该时间点是靠泊、漂移还是离泊
      if (time >= mooring && time < departure) {
        ...
        //缆绳情况
        if (hookStation) {
          singlePageRef.setCableData(hookStation);
        }
      } else {
        ...
        // singlePageRef.setDriftSwiperValue({leftValue: null, rightValue: null});
      }
    },
    /**
     * 播放按钮的点击事件
     ...
     * @param data
     */
    translateData(data = {'hook-station': [], terminal: []}) {
      const _this = this;
